...
class SONIC_PreData:
    ...
    def sampler_main(self, clip_vision,audio,image,min_resolution,expand_ratio):
        ...
        if not os.path.exists(audio2bucket_ckpt) or not os.path.exists(audio2token_ckpt) or not os.path.exists(yolo_ckpt):
        ...
        test_data = image_audio_to_tensor(face_det, feature_extractor, crop_image_pil, audio_path, limit=config.frame_num, image_size=min_resolution, area=config.area)
        ...
        ref_tensor_list,audio_tensor_list,uncond_audio_tensor_list,motion_buckets,image_embeddings=sonic_predata(whisper,audio_feature,audio_len,step,audio2bucket,clip_vision,audio2token,ref_img,image,device)
        del clip_vision,face_det,whisper
        audio2bucket.to("cpu")
        audio2token.to("cpu")
        gc.collect()
        torch.cuda.empty_cache()
        return ({"test_data":test_data,"ref_tensor_list":ref_tensor_list,"config":config,"image_embeddings":image_embeddings,
                 "audio_tensor_list":audio_tensor_list,"uncond_audio_tensor_list":uncond_audio_tensor_list,"motion_buckets":motion_buckets},)
